...
def add_option(choice_group_id):
    form = OptionForm()
    choice_group = database.session.get(ChoiceGroup, choice_group_id)
...
def list_options(choice_group_id):
    choice_group = database.session.get(ChoiceGroup, choice_group_id)
    return render_template("admin/option/list_options.html", choice_group=choice_group)


@admin.route("/edit-option/<id>", methods=["GET", "POST"])
@login_required
def edit_option(id):
    option = database.session.get(Option, id)
...
@admin.route(
    "/delete-option/<option_id>/choice_group_id/<choice_group_id>", methods=["GET"]
)
@login_required
def delete_option(option_id, choice_group_id):
    ...
    if "confirm" in request.args:
        ...
        return render_template(
            ...
            options=options,
            option=database.session.get(Option, option_id),
