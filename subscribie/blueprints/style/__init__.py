...
def inject_custom_style():
    ...
    try:
        ...
        if "primary" in css_properties_json:
        ...
        if "font" in css_properties_json:
            font = css_properties_json["font"]
    ...
    try:
        ...
        if css_style is not None and css_style.css_properties_json != "":
            try:
                ...
                if "font" in css_properties:
                    parts = css_properties["font"].split(",")
                    css_primary = f"hsl({parts[0]}, {parts[1]}, {parts[2]})"
            except Exception as e:
...
@module_style_shop.route("/style-shop-font", methods=["POST"])
@login_required
