...
async function test_order_plan_with_cancel_at(browsers, browserContextOptions) {
  for (const browserType of browsers) {
    ...
    assert(payment_content === "£10.00");
    const recuring_charge_content = await page.textContent('.Text-fontSize--16');
    assert(recuring_charge_content === "Subscribe to cancel at plan");
    ...
    await page.screenshot({ path: `trialing-subscribers-${browserType}.png` });
    ...
    assert(subscriber_plan_title_content === 'cancel at plan');

    const content_subscriber_plan_interval_amount = await page.textContent('.subscribers-plan-interval_amount');
    assert(content_subscriber_plan_interval_amount === '£10.00');
    ...
    assert(subscriber_plan_cancel_at_content === "Automatically Cancels at:");
    ...
    assert(content_upcoming_invoice_plan_price_interval === '£10.00');
    ...
    assert(subscriber_plan_title_upcoming_payment === "Plan: cancel at plan");
...
module.exports = test_order_plan_with_cancel_at
