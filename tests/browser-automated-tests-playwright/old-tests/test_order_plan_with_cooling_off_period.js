...
async function test_order_plan_with_cooling_off_period(browsers, browserContextOptions) {
  for (const browserType of browsers) {
    ...
    assert(payment_content === "10 days free");
    const recuring_charge_content = await page.textContent('.Text-fontSize--16');
    assert(recuring_charge_content === "Try Cooling off plan");
    ...
    assert(subscriber_plan_title_content === 'Cooling off plan');
    ...
    assert(subscriber_plan_trailing_content === "trialing");
...
module.exports = test_order_plan_with_cooling_off_period
